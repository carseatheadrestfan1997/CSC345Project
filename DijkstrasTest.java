	...
	public class DijkstrasTest {
	            ...
	            for (ArrayList<Integer> adjLink : adjList) {
	                ...
	                if (visited[adjLink.get(0)] == false) {
	                    if (!map.containsKey(adjLink.get(0))) {
	                        map.put(
	                            adjLink.get(0),
	                            new Node(v,distance + adjLink.get(1)));
	                    }
	                    else {
	    ...
	    static int[] dijkstraFibonacci(
		        ...
		        int S)
		    {
		        boolean[] visited = new boolean[V];
		        HashMap<Integer, Node> map = new HashMap<>();
		        FibonacciHeap<Node> q = new FibonacciHeap<>();
		 
		        map.put(S, new Node(S, 0));
		        q.insert(new Node(S, 0), 0);
		 
		        while (!q.isEmpty()) {
		            FibonacciHeap.Node<DijkstrasTest.Node> n = q.delete_min();
		            int v = n.getKey().v;
		            double distance = n.getPriority();
		            visited[v] = true;
		                ...
		                if (visited[adjLink.get(0)] == false) {
		                    if (!map.containsKey(adjLink.get(0))) {
		                        ...
		                        if (distance + adjLink.get(1)
		                            ...
		                            sn.v = v;
		                            sn.distance
		                                = (int) (distance + adjLink.get(1));
		                        }
		                    }
		                    q.insert(new Node(adjLink.get(0),
		                                   (int) (distance
		                                       + adjLink.get(1))), (distance
				                                       + adjLink.get(1)));
		                }
		            }
		        }
